...
//gain/loss occurs in a vial upon being irradiated,
//using an unirradiated vial filled with identical scintillator as a reference.
//
...
void VialDmgScaling(string DataFile, string SampleName,
		    int ThresholdIdx){
  ...
  TFile* fout = new TFile("VialDmgScalingResults.root", "RECREATE");
  ...
  for(size_t k = 0; k<RefEvts.size(); k++){
	...
	counter++;
      ...
      //The only easy way I can tell whether it is 5% WbLS or pure LS is to look
      //at the 'Dose' branch variable (as this is misread for irradiated vial
      //samples. 5% WbLS will have Dose==5, and Pure LS will have Dose==0.
	...
	TestChannel = Channel.at(TestIdx);
      ...
      else if(((TestEnd - TestIdx)<40)&&(theDose==5)){
	...
	//cout << TestIdx << "." << endl;
      ...
      //PP and HDPE vials were filled with pure LS following CERF irrad.
      else if(((TestEnd - TestIdx)<400)&&(theDose==0)){
	//cout << "Fit range is not wide enough! TestIdx = " << TestIdx 
	//   << ", TestEnd = " << TestEnd << ". Shifting TestIdx to ";
	//TestIdx = 300;
