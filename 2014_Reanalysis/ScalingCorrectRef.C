...
void ScalingCorrectRef(string DataFile, string SampleName, string RefName,
		       int ThresholdIdx){
  ...
  while(loopCtr<numEntries){
    ...
    else if(thisSampleName==(RefName)){
      NumRefs++;
  ...
  for(size_t k = 0; k<RefEvts.size(); k++){
	...
	TestChannel = Channel.at(TestIdx);
      ...
      else if(((TestEnd - TestIdx)<40)&&(theIDnum<60)&&(theIDnum!=40)){
	...
	cout << TestIdx << "." << endl;
      ...
      else if(((TestEnd - TestIdx)<60)&&(theIDnum==100)){
	cout << "Fit range is not wide enough! TestIdx = " << TestIdx 
	     << ", TestEnd = " << TestEnd << ". Shifting TestIdx to ";
	TestIdx = TestIdx - (60 - (TestEnd-TestIdx));
	cout << TestIdx << "." << endl;
      }
      else if(((TestEnd - TestIdx)<80)&&(theIDnum==140)){
	cout << "Fit range is not wide enough! TestIdx = " << TestIdx 
	     << ", TestEnd = " << TestEnd << ". Shifting TestIdx to ";
	TestIdx = TestIdx - (80 - (TestEnd-TestIdx));
	cout << TestIdx << "." << endl;
      }
      else if(((TestEnd - TestIdx)<400)&&(theIDnum==1000)){
