...
//SampleNames.txt and, for each of these run CorrectRef.C.
...
void Run_CorrectRef(string datafile, string samplefile){
  ...
  while(std::getline(ifst,line)) {
    ...
    command = ".x CorrectRef_Scaled.C+(\"" + datafile + "\", \"" + line + "\")";
